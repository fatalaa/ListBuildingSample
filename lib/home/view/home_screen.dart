  ...
  void initState() {
    ...
    _homeBloc.scannedItemsStream.listen((event) {
      setState(() {
        ...
        _sparkScanView?.emitFeedback(new SparkScanViewSuccessFeedback(null));
        _scannedItems.add(event);
      });
    }, onError: (error) {
      // Emit sound and vibration feedback
      _sparkScanView?.emitFeedback(SparkScanViewErrorFeedback(error, Duration(seconds: 60), null));
    });
  }
